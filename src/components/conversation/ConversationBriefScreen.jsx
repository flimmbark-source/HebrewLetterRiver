...
export default function ConversationBriefScreen({ scenario, onStartSegment, onBack }) {
  ...
  return (
    <div className="fixed inset-0 z-30 overflow-hidden bg-[#fbf4e4] text-[#173d2e]">
      <div className="relative mx-auto flex h-[100dvh] w-full max-w-[430px] flex-col overflow-hidden bg-[#fbf4e4] px-5 pb-[calc(var(--bottom-nav-safe-space)+8px)] pt-10 md:max-w-[920px] md:px-8 md:pb-[calc(var(--bottom-nav-height)+28px)]">
        ...
        <header className="relative z-10 grid grid-cols-[2.5rem_1fr_2.5rem] items-center md:mx-auto md:w-full md:max-w-[640px]">
          <button
            onClick={onBack}
        ...
        <main className="relative z-10 flex min-h-0 flex-1 flex-col pt-3 md:pt-8">
          <div className="mt-auto md:mt-0 md:grid md:min-h-0 md:flex-1 md:grid-cols-[minmax(0,1fr)_240px] md:items-end md:gap-6">
            <div className="min-w-0">
              <section className="text-center md:mx-auto md:max-w-[640px]">
            ...
            {canSelectSegments && (
              <aside className="mt-3 md:mb-1 md:mt-0 md:flex md:h-full md:items-end">
                <button
                  type="button"
                  onClick={handleBeginRoute}
                  disabled={!selectedRouteStop}
                  className="relative z-10 flex w-full items-center justify-center gap-2 rounded-2xl px-5 py-3.5 text-lg font-bold text-white shadow-lg transition hover:brightness-105 active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-60 md:min-h-[96px] md:flex-col md:px-5 md:py-5 md:text-xl"
